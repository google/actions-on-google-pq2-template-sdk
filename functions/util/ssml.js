...
/**
 * Merges ssml strings with custom mark.
 * @param {Array<string>} parts - Array of ssml parts.
 * @param {string} markName - Custom mark name.
 * @param {number} breakTime - Break time in milliseconds to insert between parts.
 * @return {string} - Merged ssml string.
 */
const mergeWithMark = (parts, markName = TtsMark.FLIP, breakTime = config.SSML_BREAK_TIME) => {
  parts = parts.map((entry) => entry.trim()).filter(Boolean);
  let merged = parts[0];
  let count = 1;
  for (let i = 1; i < parts.length; ++i) {
    merged += `<break time="${breakTime}ms"/><mark name="${markName}_${count++}"/> ${parts[i]}`;
  }
  merged = merged.replace(/<speak>|<\/speak>/gi, '');
  return `<speak>${merged}</speak>`;
};
