...
describe('util - ssml', function() {
  ...
  describe('mergeWithMark', function() {
    const parts = ['<speak>a</speak>', '<speak>b</speak>'];
    const markName = 'SWITCH';
    const breakTime = 500;

    it('returns a string', function() {
      const output = ssml.mergeWithMark(parts, markName, breakTime);
      expect(output).to.be.a('string');
    });

    it('surrounds by <speak> tag', function() {
      const output = ssml.mergeWithMark(['a', 'b'], markName, breakTime);
      expect(output).to.eql(
        `<speak>a<break time="${breakTime}ms"/><mark name="${markName}_1"/> b</speak>`
      );
    });
    ...
    it('filters trimmed empty parts', function() {
      const output = ssml.mergeWithMark(['  ', ' ', ' a ', ''], markName, breakTime);
      expect(output).to.eql(`<speak>a</speak>`);
    });

    it('removes inner <speak> tags from input strings', function() {
      const output = ssml.mergeWithMark(
        ['<speak>a</speak>', '<speak>b</speak>'],
        markName,
        breakTime
      );
      expect(output).to.eql(
    ...
    it('increment custom mark name for each additional interval', function() {
      const output = ssml.mergeWithMark(['a', 'b', 'c'], markName, breakTime);
      expect(output).to.eql(
        `<speak>a<break time="${breakTime}ms"/><mark name="${markName}_1"/> b` +
          `<break time="${breakTime}ms"/><mark name="${markName}_2"/> c</speak>`
      );
    });

    it('Use default markName and breakTime if not provided', function() {
      const output = ssml.mergeWithMark(['a', 'b']);
      expect(output).to.eql(
        `<speak>a<break time="${config.SSML_BREAK_TIME}ms"/><mark name="${
          TtsMark.FLIP
        }_1"/> b</speak>`
      );
    });
