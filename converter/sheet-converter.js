...
class SheetConverter {
  ...
  async parseArrayTab(tab) {
    ...
    for (const colDef of tab.columns) {
      const colIdx = rows[headerRowIdx].findIndex(
        (val) => val.toLowerCase() === colDef.displayName.toLowerCase()
      );
  ...
  async parseDictTab(tab) {
    ...
    for (let r = headerRowIdx + 1; r < rows.length; ++r) {
      ...
      let key = '';
      for (let c = 0; c < row.length; ++c) {
        if (c !== keyColIdx && !idxToColDef[c]) continue;
        if (c === keyColIdx) {
          const keyDef = tab.keys.find(
            (def) => def.displayName.toLowerCase() === row[c].toLowerCase()
          );
          if (!keyDef) continue;
          key = keyDef.name;
        } else {
          const colDef = idxToColDef[c];
          doc[colDef.name] = row[c].trim();
        }
      }
      if (!key) continue;
      output[key] = doc;
